...
async function getFilesList(dir_path, cmd) {
  let args;
  let exec;
  if (cmd == "rg") {
    exec = cmd;
...
var CryptoHandler = class {
  ...
  async encryptAllFiles() {
    ...
    const fl = await getFilesList(import_coc3.workspace.root, "find");
    if (!fl) {
  ...
  isAutoEncrypt() {
    return this.setting.autoEnc != void 0 && this.setting.autoEnc;
  }
};
